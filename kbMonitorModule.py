...
def slider(ax, label, valmin, valmax, valinit=0.5, valfmt='%1.2f',
    user_callback=None,
    closedmin=True, closedmax=True, dragging=True, valstep=None):
    """
    slid = slider(ax, label, valmin, valmax, valinit=0.5, valfmt='%1.2f',
        user_callback=None,
        closedmin=True, closedmax=True, dragging=True, valstep=None)

Puts up, in the passed axes ax, a sldier box with label (a string)
to its left. The slider goes from valmin to valmax, and also shows its
value in text form to the right (with format valfmt). closedmin and
closedmax are Booleans that indicate whether the interval encoded is
closed or open. dragging is a Boolean that indicates whether mouse
dragging of the value is allowed. And valstep can be used to quantize
changes in the slider.

If user_callback is defined, it must be a function of one parameter,
which will be called when the slider is changed, with the value of the slider
as the parameter.

To access the value from Julia use the Pycall syntax

    slid.val    (in Python, slid.val)

and to set the value from Julia use

    slid.set_val(new_value)      (in Python, slid.set_val(new_value))

    """
    slid = Wid.Slider(ax, label, valmin, valmax, valinit=valinit, valfmt=valfmt,
    closedmin=closedmin, closedmax=closedmax, dragging=dragging, valstep=valstep)
    if user_callback != None:
        slid.on_changed(user_callback)
...
def check_buttons(rax, labels, actives, user_callback=None):
    """
    chb = check_buttons(ax, labels, actives, user_callback=None)
